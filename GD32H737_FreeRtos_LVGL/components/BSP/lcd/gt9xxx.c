...
uint8_t gt9xxx_init(void)
{
    ...
    rcu_periph_clock_enable(GT9XXX_RST_GPIO_CLK);     /* 使能RST引脚时钟 */
    rcu_periph_clock_enable(GT9XXX_INT_GPIO_CLK);     /* 使能INT引脚时钟 */

    /* 设置GT_RST引脚 推挽输出模式 */
    gpio_mode_set(GT9XXX_RST_GPIO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GT9XXX_RST_GPIO_PIN);
    gpio_output_options_set(GT9XXX_RST_GPIO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_60MHZ, GT9XXX_RST_GPIO_PIN);
  
    gpio_mode_set(GT9XXX_INT_GPIO_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, GT9XXX_INT_GPIO_PIN);  /* GT_INT引脚模式设置,上拉输入 */
    ...
    gpio_mode_set(GT9XXX_INT_GPIO_PORT, GPIO_MODE_INPUT, GPIO_PUPD_NONE, GT9XXX_INT_GPIO_PIN);    /* GT_INT引脚模式设置,浮空输入 */
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            ...
            for (i = 0; i < g_gt_tnum; i++)
            {
                if (tp_dev.sta & (1 << i))                      /* 触摸有效? */
                {
                    ...
                    if (lcddev.id == 0X5510 || lcddev.id == 0X5310 || lcddev.id == 0X7796 || lcddev.id == 0X7789)     /* MCU屏或SPI串口屏 */
                    {
