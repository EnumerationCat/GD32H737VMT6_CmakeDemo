...
{	
    rcu_periph_clock_enable(CT_IIC_SCL_GPIO_CLK);  /* SCL引脚时钟使能 */
    rcu_periph_clock_enable(CT_IIC_SDA_GPIO_CLK);  /* SDA引脚时钟使能 */

    /* SCL引脚模式设置,推挽输出,上拉 */
    gpio_mode_set(CT_IIC_SCL_GPIO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, CT_IIC_SCL_GPIO_PIN);
    gpio_output_options_set(CT_IIC_SCL_GPIO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_60MHZ, CT_IIC_SCL_GPIO_PIN);

    /* SDA引脚模式设置,开漏输出,上拉, 这样就不用再设置IO方向了, 开漏输出的时候(=1), 也可以读取外部信号的高低电平 */
    gpio_mode_set(CT_IIC_SDA_GPIO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, CT_IIC_SDA_GPIO_PIN);
    gpio_output_options_set(CT_IIC_SDA_GPIO_PORT, GPIO_OTYPE_OD, GPIO_OSPEED_60MHZ, CT_IIC_SDA_GPIO_PIN);
